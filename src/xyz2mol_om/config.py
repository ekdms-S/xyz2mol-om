...
#   🔴 **`k = 1` 은 부호로 갈린다.**
#     · **반대 부호**는 건드리지 않는다 — 일산화탄소 `[C⁻]≡[O⁺]` · 아민 옥사이드 `R₃N⁺–O⁻` ·
#       인 일리드 `R₃P⁺–C⁻`. 여기서 차수를 옮기면 금속 카보닐 전체가 무너진다.
#     · **같은 부호**는 올린다 (2026-09-10 추가). 결합 하나를 올리면 **양 끝이 동시에** 중성이
#       된다: `[C⁻](H)(H)[O⁻]` 는 폼알데하이드 `H₂C=O`, `[C⁻]([O⁻])(H)CH₃` 는 아세트알데하이드다.
#       Gold-DIGR 의 IRC 끝점에서 **떨어져 나온 자유 분자**가 실제로 이렇게 나왔고, 금속이 없어
#       산화수가 차이를 흡수해 주지도 않는다.
#       ⚠️ 예외는 **과산화물** `[O⁻]–[O⁻]` — 실재하는 화학종이고 금속 착물의 흔한 리간드라
#       막는다. `[C⁻]–[C⁻]`(에텐) 은 올리는 것이 맞으므로 O–O 만 막는다.
#       holdout: 8 구조가 바뀌어 `Σq_L` **틀→맞 2 · 맞→틀 1**,
#       `Sq_L` .8587 → **.8596** · `OS` .8938 → **.8946** · 나머지 전 지표 불변.
...
# `SIGCUT` — `QSHIFT` 의 `k = 1` 같은 부호 분기가 **σ M–L 예산 때문에만** 막혔을 때, 그 σ M–L 을
#   끊고 결합을 올린 뒤 **다시 푼다** (`charge.formal.sigma_ml_blocking_cancel` + `api.predict`).
#   근거: σ M–L 은 배위 원자의 내부 원자가를 **하드 제약**으로 1 먹는데, 그 때문에 생기는 형식전하는
#   **소프트 비용**일 뿐이라 "차수 올리고 중성" 과 "차수 낮추고 전하 둘" 의 비교가 애초에 일어나지
#   않는다 (`dev/docs/failures/260910_sigma_ml_eats_valence.md`).
#   판정 조건 — 전부 구조적이고 적합값이 없다:
#     · 인접한 두 원자의 형식전하가 **같은 부호로 음수**이고, 결합을 1 올리면 둘 다 중성이 된다
#     · 상한 검사가 `bml` **때문에만** 실패한다 (여유가 있으면 `QSHIFT` 자리다)
#     · σ M–L 을 **하나** 끊으면 상한에 들어온다
#     · 짝 원자가 **같은 금속에 배위하지 않는다** 🔴 — 둘 다 배위하면 그것은 진짜 이음이온
#       리간드다 (벤자인 착물 · 알카인에서 온 메탈라사이클로프로펜 · C,C-킬레이트).
#       Gold-DIGR 실측으로 인접 음이온 쌍의 23.2% 가 이 부류다.
#     · `[O⁻]–[O⁻]` 과산화물은 `QSHIFT` 와 같은 이유로 제외한다
#   끊은 뒤 **|전하| 합이 실제로 줄 때만** 채택하고, 아니면 원래 답으로 되돌린다.
#   Gold-DIGR 실측 (21,196 프레임 · `dev/analysis/scratch/260911_adjacent_anion_pairs.py`):
#     인접 음이온 쌍 1,824 건(과산화물 제외) 중
#       σ M–L 을 끊어야 올릴 수 있다   1,184 (64.9%)  ← 이 규칙
#       짝도 결합 (진짜 이음이온)         424 (23.2%)  ← 제외됨
#       상한 여유 있음                   17 ( 0.9%)  ← `QSHIFT` 가 이미 했어야 하는 것
#     원소 조합은 C–C 878 · C–O 279 · C–N 24 로 **탄소에 국한되지 않는다.**
#   ⚠️ 끊으면 조각이 금속에서 완전히 떨어지는 경우가 34.5% 다. `AGOC` 와 달리 가드를 두지 않는다 —
#      IRC 끝점에는 아직 결합하지 않은 co-reactant 가 근처에 떠 있을 수 있고, 억지로 붙여두면 실제
#      회합 반응을 지운다 (오너 판단 2026-09-11).
#   ── 실측 (holdout 6,793 · 2026-09-11 · `SIGCUTW=0.40`) ──────────────────────────────
#     OS            .8946 → **.8960**   틀→맞 **4** · 맞→틀 **0**
#     T4 결합 유무    .9917 → **.9918**   거짓양성 −8 · 참양성 −4  (순증)
#     T3 `Double`   .7747 → **.7753**
#     T1 · T3 나머지 세 클래스 · T5 · T6 · T8 세 클래스 · `Sq_L` · 위반 — **전부 불변**
#     출력이 바뀐 구조는 **11 개**뿐이다.
#   ⚠️ 상한 없이 재면 T4 가 .9917 → .9911 로 떨어진다 (612 구조 변화 · 참양성 −76). 그래서
#      `SIGCUTW` 가 규칙의 일부다 — 자세한 것은 그 항목.
#   Gold-DIGR (21,196 프레임, 상한 적용 후 추정): 866 프레임(4.1%)에서 발동하고, 발동 반응
#     793 건의 `max|ΔOS| >= 2` 가 551 → 281 로 준다.
#   **On by default** — 오너 채택 2026-09-11.
SIGCUT = os.environ.get("SIGCUT", "1") == "1"

# `SIGCUTW` — `SIGCUT` 이 끊을 수 있는 σ M–L 의 **Mayer 상한**. 이보다 세면 손대지 않는다.
...
#   ⚠️ `WMIN` 과 다르다. 저쪽은 M–L 후보 자체를 **전역으로** 거르는 하한이라 haptic(원래 약하다)을
#   같이 죽여 T5 가 .9778 → .7241 로 무너졌다. 이쪽은 **이미 받아들인 σ 를 끊을지**만 정한다.
#   Mayer 가 없으면 끊지 않는다.
SIGCUTW = float(os.environ.get("SIGCUTW", "0.40"))

# `SIGETA` — `SIGCUTW` 상한에 걸려 **끊지 못한** σ M–L 을, 끊는 대신 **η² 로 돌린다**
#   (그 결합과 짝 원자를 함께 haptic 으로 만들고 차수를 올린다).
#   오너 제안 (2026-09-13): "Mayer 0.4 이상인 것들은 아 그럼 eta2 인가보다 하고 negative charge
#   를 없애는 방식으로 결합 차수를 올린 다음에 dative -> haptic 으로 변환하면 안 되나".
#   🔴 **짝을 고를 여지가 없다.** 발동 조건이 «음이온 쌍 `X–Y` 의 차수를 올려야 하는데 `X` 의
#   σ M–L 이 막는다» 이므로, 짝은 올리려는 그 결합의 반대쪽 끝 `Y` 로 이미 정해져 있다.
#   («어느 이웃이냐» 가 문제가 되는 것은 `ETA2NEAR` 처럼 조건 없이 T4 문턱을 풀 때다.)
#   장점: **M–L 을 끊지 않으므로 T4 참양성을 잃지 않는다** — `SIGCUTFIT` 이 지금 내주는 14 개.
#   대가: CSD 가 `Single` 이라 부르는 M–L 을 `Pi` 로 바꾸게 되므로 T5·T8 이 흔들린다.
#   🔴 **측정 후 미채택 (2026-09-13). 기본값 off.** 발동 자체가 거의 없다.
#     holdout 6,793: **구조 1 개**만 바뀌고 모든 지표가 소수점 넷째 자리까지 불변 — 그 1 개에서
#       T4 거짓양성 +1 · haptic 거짓양성 +2 (즉 소폭 마이너스).
#     Gold-DIGR `H1_starved_mayer` 8 건: **1 건만 개선** (`Pt +6 -> +4`, 조각 −2 -> 0).
#   왜 안 걸리나 — `SIGCUTFIT` 이 먼저 거리로 판정해서 «올려라» 인 자리는 이미 끊는다. 남는 것은
#   Mayer 도 세고 거리도 올리라고 하지 않는 자리뿐이고, 거기서 차수를 올리는 것은 기하를 거스르는
#   일이라 `abs_charge_sum` 검사에서 대부분 되돌려진다.
#   ⚠️ 그리고 애초에 대상이 없다. 인접 음이온 쌍에서 **짝이 T4 를 통과하고 Mayer >= 0.40** 인
#     경우가 Gold-DIGR 21,196 프레임에서 **2 건**이다 (거리 탈락 · Mayer < 0.40 이 24 건으로
#     대부분 — 짝이 3.1~3.2 Å 로 정말 멀다). η² 로 묶을 대상 자체가 드물다.
SIGETA = os.environ.get("SIGETA", "0") == "1"

# `SIGCUTFIT` — `SIGCUTW` 상한 **예외**: Mayer 가 상한 위여도, 차수를 올린 쪽이 그 원소쌍의
#   **클래스 길이 중앙값**(`scores4` 의 `med`)에 더 맞으면 끊는다. 문턱이 아니라 두 중앙값 중
#   어느 쪽이 가까운가의 비교라 **새 상수가 없다.**
#   ⚠️ 끊는 쪽은 **두 조건**을 요구한다 — 올린 쪽이 더 가깝고, **현재 차수가 그 분포 밖**
#      (`|d − med[cur]| > scl[cur]`). `QSHIFT`/`QGEM` 은 M–L 을 끊지 않으므로 앞의 조건만 쓴다.
#   실측 (holdout 6,793 · `QGEM` 위에 얹어서): `OS` .8964 → **.8971** (틀→맞 2) ·
#     T4 .9918 → **.9916** (참양성 −14 · 거짓양성 −1) · `Sq_L` .8596 불변 · 나머지 전부 불변.
#   🔴 대가는 **CSD 가 결합이라 부르는 M–L 14 개**다. 그래도 채택한 이유는 이것이 겨냥하는 것이
#     `Double` 로 눌린 알카인(오너 지목 `A_dOS2_ligand_only__06`: C–C **1.209 Å** · Pd–C Mayer
#     0.622)처럼 **TS 근방 기하**라서다 — Mayer 만으로는 실재 결합과 구별되지 않고, 리간드 자신의
#     길이가 유일한 증거다. 같은 모양의 판단을 `AGOC` 에서도 했다.
SIGCUTFIT = os.environ.get("SIGCUTFIT", "1") == "1"

# `QGEM` — `QSHIFT` 의 **제미널** 분기: 공통 이웃 하나를 사이에 둔 같은 부호 음이온 쌍의 두
#   결합을 **같이** 올린다. 교대(±1)로는 원리상 못 고치는 꼴이다 (`[O⁻]–C–[O⁻]` = CO₂).
#   길이가 두 결합 다 올린 차수에 맞을 때만 움직인다.
#   **공짜다.** holdout: `OS` .8960 → **.8964** (틀→맞 1 · 맞→틀 0) · 바뀐 구조 **2 개** ·
#   T4 참양성 손실 **0** · 다른 지표 전부 불변. 금속 없는 자리에서도 고친다 — 나이트로메탄이
#   `N([O⁻])[O⁻]`(전하 −2) 대신 교과서 구조 `C[N⁺](=O)[O⁻]` 로 나온다 (`dev/tests/test_radical.py`).
QGEM = os.environ.get("QGEM", "1") == "1"

# `QCHFIT` — `QSHIFT` 의 **이음이온 킬레이트 게이트 예외**: 두 음이온이 다 금속에 배위해도,
#   주개-주개 결합 길이가 올린 차수에 더 맞으면 움직인다.
#   🔴 **측정 후 기각 (2026-09-11). 기본값 off.** holdout 에서 `Sq_L` **.8596 → .8579**
#   (맞→틀 2)로 떨어지고 `OS` 는 +1 밖에 못 얻는다. 이음이온 킬레이트 게이트는 무디지만
#   **그 무딤이 값을 한다.**
#   ⚠️ 이 스위치를 만들게 한 두 사례(`A_dOS2_ligand_only__01` Ni `[HN⁻][O⁻]` · `__04` Fe
#      `[HN⁻][N⁻H]`)는 **켜도 안 움직인다.** 길이가 실제로는 그것을 지지하지 않기 때문이다 —
#      CSD 적합 중앙값으로 리간드 `N–N` 단일은 **1.339 Å**, `N–O` 단일은 **1.291 Å** 이고
#      (자유 분자의 1.45 가 아니다 — 리간드의 N–N·N–O 단일은 대부분 공액 환경에 있다),
#      관측 1.293·1.336 은 `Double` 중앙값(1.171·1.210)보다 `Single` 쪽에 가깝다.
#      ⇒ 그 두 건이 틀렸다는 근거 자체가 없다. 규칙은 증거 없이 움직이지 않은 것이다.
